...
Token = collections.namedtuple('Token', ['name', 'value']) #именованный кортеж, обращаемся по аттрибутам
...
def match(rule_name, tokens):
    if tokens and rule_name == tokens[0].name:  # Match a token? #Если переменная задана "if tokens" и она не является 
...
def calc(expr):
    split_expr = re.findall(r'[\d.]+|[\+\-\/\*\(\)]', expr)
    tokens = [Token(token_map.get(x, 'NUM'), x) for x in split_expr] #список из имен. кортежей и токенов
